...
class Decoder(nn.Module):
    def __init__(
        ...
        if enable_attention:
            ...
            def dummy_fn(x: torch.Tensor, y: torch.Tensor):
                return x
            self.attention = dummy_fn
    ...
    def forward(
        ...
        if self.training:
            dropout_mask = torch.bernoulli(
                torch.full_like(dec_h, 1.0 - self.dropout_p))
        ...
        for step in range(self.max_len):
            ...
            # Forward propagation
            if self.isLSTM:
                dec_h, dec_c = self.rnn(input, (dec_h, dec_c))
            else:
                dec_h = self.rnn(input, dec_h)
            ###########################
            # Apply Dropout mechanism
            dec_h = dec_h * dropout_mask
            #############################
            # Apply Attention mechanism
            dec_h_for_output = self.attention(dec_h, enc_hs)
            ################
            # Output layer
            output = self.out(dec_h_for_output)
            #######################
            # Define a next input
            if self.training:
                assert gold_output_seq is not None
                input = self.emb(gold_output_seq[:, step])
            else:
                input = self.emb(output.argmax(dim=1))
            ###########################
            # Make an output sequence
            output_seq.append(output)

        output_seq = torch.stack(output_seq).permute(1, 0, 2)

        dummy = torch.zeros(batch_size).to(device)

        return output_seq, dummy, dummy
...
class Decoder_REINFORCE(Decoder):
    ...
    def forward(
        ...
        for _ in range(self.max_len):
            ...
            dec_h_for_output = self.attention(dec_h, enc_hs)
            # Output layer
            distr = Categorical(logits=self.out(dec_h_for_output))
            output = self.sample_from(distr)
            # Define next input
            input = self.emb(output)
...
class Discriminator(nn.Module):
    ...
    def forward(self, encoder_hs, encoder_last_h, candidates):
        #############
        # Meta Info #
        #############
        ...
        if self.impatient:
            score = torch.einsum('bnh,bth->bnt', candidates_h, encoder_hs)
        else:
            score = torch.einsum('bnh,bh->bn', candidates_h, encoder_last_h)
        dummy = torch.zeros(batch_size).to(device)
