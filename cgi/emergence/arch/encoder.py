...
class Encoder(nn.Module):
    ...
    def forward(self, input: torch.LongTensor):
        ...
        if self.no_dropout:
            ...
            #####################
            if self.training:
                dropout_mask = torch.bernoulli(torch.full_like(old_h, 1.0 - self.dropout_p))
            ...
            for batch_size in packed.batch_sizes.tolist():
                ...
                # Forward propagation
                if self.isLSTM:
                    new_h = self.rnn(step_input, (old_h[:batch_size], old_c[:batch_size]))
                    new_h, new_c = new_h
                else:
            ...
            if self.isLSTM:
                last_c = torch.index_select(old_c, 0, packed.unsorted_indices)
